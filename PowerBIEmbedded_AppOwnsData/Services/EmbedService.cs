...
namespace PowerBIEmbedded_AppOwnsData.Services
{
    public class EmbedService : IEmbedService
    {
        ...
        public List<ReportDefinition> ReportList
        {
        ...
        private List<ReportDefinition> m_ReportList;
        ...
        public async Task<bool> GetReportList(string groupId)
        {
            ...
            try
            {
                using (var client = new PowerBIClient(new Uri(ApiUrl), m_tokenCredentials))
                {
                    ...
                    var items = reports.Value;
                    var mylist = new List<Tuple<string, string>>();
                    foreach (var item in items)
                    {
                        m_ReportList.Add(new ReportDefinition(item.Id, item.Name));
                    }
                    return true;
                }
            }
        ...
        public async Task<bool> EmbedReport(string username, string roles, string groupId, string reportId)
        {
            ...
            try
            {
                ...
                using (var client = new PowerBIClient(new Uri(ApiUrl), m_tokenCredentials))
                {
                    ...
                    if (!string.IsNullOrWhiteSpace(username))
                    {
                        ...
                        generateTokenRequestParameters = new GenerateTokenRequest(accessLevel: "edit", allowSaveAs: true, datasetId: report.DatasetId, identities: new List<EffectiveIdentity> { rls });
                    }
                    else
                    {
